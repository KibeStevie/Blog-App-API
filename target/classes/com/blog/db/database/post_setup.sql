...
BEGIN
  ...
  IF v_post_owner IS NOT NULL AND v_post_owner != NEW.user_id THEN
    ...
    VALUES (
      ...
      NEW.user_id,     -- actor (who liked the post)
      'like',
      NEW.post_id,     -- reference_id = post_id
      'Liked your post'
    );
  END IF;
  RETURN NEW;
...
BEGIN
  ...
  IF NEW.parent_comment_id IS NOT NULL THEN
    ...
    IF v_parent_author IS NOT NULL AND v_parent_author != NEW.user_id THEN
      INSERT INTO notifications (user_id, actor_id, type, reference_id, message)
      VALUES (
        v_parent_author,  -- recipient (parent comment author)
        NEW.user_id,      -- actor (who replied)
        'comment_reply',
        NEW.post_id,      -- reference_id = post_id
  ...
  ELSE
    ...
    IF v_post_owner IS NOT NULL AND v_post_owner != NEW.user_id THEN
      ...
      VALUES (
        v_post_owner,     -- recipient (post owner)
        NEW.user_id,      -- actor (who commented)
        'comment',
        NEW.post_id,      -- reference_id = post_id
...
BEGIN
  ...
  IF v_post_owner IS NOT NULL AND v_post_owner != NEW.user_id THEN
    ...
    VALUES (
      v_post_owner,    -- recipient (post owner)
      NEW.user_id,     -- actor (who bookmarked)
      'bookmark',
      NEW.post_id,     -- reference_id = post_id
...
BEGIN
  ...
  WHERE reference_id = OLD.post_id 
    AND type IN ('like', 'comment', 'bookmark', 'comment_reply');
  RETURN OLD;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  -- ✅ 3. Return paginated posts for the authenticated user
  RETURN QUERY
  SELECT 
    p.post_id, 
    p.title, 
  ...
  WHERE p.user_id = v_user_id  -- ✅ Use resolved user ID
...
BEGIN
  ...
  IF v_exists THEN
    ...
    DELETE FROM notifications WHERE actor_id = v_user_id AND reference_id = p_post_id AND type = 'like';
...
BEGIN
  ...
  IF v_exists THEN
    ...
    DELETE FROM notifications WHERE actor_id = v_user_id AND reference_id = p_post_id AND type = 'bookmark';
