...
RETURNS TABLE(success BOOLEAN, session_id UUID, user_id INT, message TEXT, error TEXT) AS $$
DECLARE
  v_user_id INT;
BEGIN
  SELECT users.user_id INTO v_user_id FROM users 
  WHERE users.email = p_email AND users.password_hash = p_password_hash;

  IF v_user_id IS NULL THEN
    RETURN QUERY SELECT FALSE, NULL::UUID, NULL::INT, NULL::TEXT, 'Invalid credentials'::TEXT;
    RETURN;
  END IF;

DELETE FROM user_sessions WHERE user_id = v_user_id;

  -- session_id auto-generated by DEFAULT gen_random_uuid()
  INSERT INTO user_sessions (user_id, expires_at)
  VALUES (v_user_id, CURRENT_TIMESTAMP + INTERVAL '30 days')
  RETURNING user_sessions.session_id INTO session_id;

  RETURN QUERY SELECT TRUE, session_id, v_user_id, 'Login successful'::TEXT, NULL::TEXT;
END;
$$ LANGUAGE plpgsql;
...
-- 5.6 GET FOLLOWERS
CREATE OR REPLACE FUNCTION fn_get_followers(p_user_id INT)
RETURNS TABLE(user_id INT, username VARCHAR, profile_image TEXT, created_at TIMESTAMP) AS $$
BEGIN
  RETURN QUERY
  SELECT u.user_id, u.username, u.profile_image, f.created_at
  FROM user_follows f
...
-- 5.7 GET FOLLOWING
CREATE OR REPLACE FUNCTION fn_get_following(p_user_id INT)
RETURNS TABLE(user_id INT, username VARCHAR, profile_image TEXT, created_at TIMESTAMP) AS $$
BEGIN
  RETURN QUERY
  SELECT u.user_id, u.username, u.profile_image, f.created_at
